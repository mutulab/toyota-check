...
if check_type == "📖 マニュアル":
    ...
    st.info(
        ...
        "サイトマップ管理・バックグラウンドクロールは実行ボタン不要で画面から直接操作します。"
    )

    # ── サイトマップ管理 ────────────────────────────────────────
    with st.expander("### 🗂️ サイトマップ管理（マスタ運用）", expanded=True):
        st.markdown("""
コンテンツ管理表をアプリ内に**マスタとして保持**し、閲覧・編集・更新する管理機能です。

**初回セットアップ（1回だけ）**
1. 「📥 Excelからマスタを取り込み」で tjpコンテンツ管理表.xlsx をアップロード
2. 「💾 マスタとして保存」→ 以降はアップロード不要（開くだけでマスタが読み込まれます）

**3つの表示モード**
| モード | 用途 |
|---|---|
| 🌲 ツリービュー | サイト構造をインデント表示。表示階層（第1〜第6）で折り畳み。畳んだ配下は親行に件数表示 |
| ✏️ 一覧編集 | Excel風の編集。ページング（200〜2000行）対応。「💾 編集内容をマスタに保存」で確定 |
| 📊 ディレクトリ集計 | 指定階層までのフォルダ単位にページ数・HTML/アプリ内訳を集計 |

**種別判定**: URLルールで HTML（静的）／アプリ（動的）／ファイルを自動判別します。

**🕷️ クロール差分検知**: サイトを巡回して一覧に無いURLを検知 → チェックした行をマスタに追加
（追加日・追加元を自動記録）。

**📅 追加URLの差分管理**: 追加日別のサマリー・絞り込み・取り消し削除・CSV出力。

**🏝️ 陸の孤島チェック**: 全ページを巡回して被リンク数を集計し、どこからもリンクされて
いないURLを検出。前回実行との差分（新たに孤島／解消）も表示。
※JS生成リンクは検出できないため判定は要検証。

**保存とバックアップ**: 保存のたびにアプリ内ストレージ＋GitHub（Secretsに
`GITHUB_TOKEN` 設定時）へ自動保存。再デプロイ後もGitHubから自動復元されます。
更新履歴（日時・操作・件数）は「🕘 更新履歴」で確認できます。

**バックグラウンドクロールとの連携**: クロールのURL取得方法で
「🗂️ サイトマップマスタ」を選ぶと、このマスタを対象に各種チェックを実行できます。
""")
